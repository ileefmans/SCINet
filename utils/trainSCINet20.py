...
def get_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="1 if running on local machine, 0 if running on AWS",
    )
    parser.add_argument(
        "--local_pickle_path",
        type=str,
        default="/Users/ianleefmans/Desktop/Insight/Project/Re-Identifying_Persistent_Skin_Conditions/skinConditionDetect/pickle/simple_train_dict.pkl",
        help="path to local pickled annotation path dictionary",
    )
    parser.add_argument(
        "--remote_pickle_path", type=str, default="simple_train_dict.pkl"
    )
    parser.add_argument(
        "--local_val_pickle_path",
        type=str,
        default="/Users/ianleefmans/Desktop/Insight/Project/Re-Identifying_Persistent_Skin_Conditions/skinConditionDetect/pickle/simple_train_dict.pkl",
        help="path to local val pickled annotation path dictionary",
    )
    parser.add_argument(
        "--remote_val_pickle_path", type=str, default="simple_val_dict.pkl"
    )
    parser.add_argument(
        "--local_data_directory",
        type=str,
        default="/Users/ianleefmans/Desktop/Insight/Project/Data",
        help="Path to data",
    )
    parser.add_argument(
        ...
        type=str,
        default="<blank>",
        help="no remote data dictionary applicable",
    )
    parser.add_argument(
        "--image_size",
        type=tuple,
        default=(256, 256),
        help="Size all images will be transformed to (height,width)",
    )
    ...
    parser.add_argument(
        ...
        type=bool,
        default=True,
        help="True if dataloader shuffles input samples before batching, False if samples are batched in order",
    )
    parser.add_argument(
        "--geometric",
        type=bool,
        default=False,
        help="True if geometric mapping, False if deep learning mapping",
    )
    parser.add_argument(
        "--learning_rate",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of epochs model will be trained for",
    )
    parser.add_argument(
        "--local_save_path",
        type=str,
        default="/Users/ianleefmans/Desktop/Insight/Project/Data/model_checkpoints",
        help="Path to folder to save model parameters after trained",
    )
    parser.add_argument(
        "--load_weights",
        type=bool,
        default=False,
        help="Determines whether or not pretrained weights will be loaded during training",
    )
    parser.add_argument("--access_key", type=str, default="", help="AWS Access Key")
    parser.add_argument(
        "--secret_access_key", type=str, default="", help="AWS Secret Access Key"
    )
...
class Trainer:
    ...
    def __init__(self):
        ...
        self.valset = CreateDataset(
            self.val_pickle_path,
            self.data_directory,
            img_size=self.img_size,
            local=self.local,
            access_key=self.access_key,
    ...
    def loss_fcn(self, x1, x2, x1_hat, x2_hat, z1, z2):
        BCEx1 = nn.functional.binary_cross_entropy(
            x1_hat.view(-1, 3 * 256 * 256), x1.view(-1, 3 * 256 * 256), reduction="sum"
        )
        BCEx2 = nn.functional.binary_cross_entropy(
            x2_hat.view(-1, 3 * 256 * 256), x2.view(-1, 3 * 256 * 256), reduction="sum"
        )
        BCEz1z2 = nn.functional.binary_cross_entropy(
            z1.view(-1, 20 * 61 * 61), z2.view(-1, 20 * 61 * 61), reduction="sum"
        )
