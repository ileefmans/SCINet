...
class Image_Process:
    ...
    def expand(self, image, resample=0):
        ...
        if new_width <= self.new_size[1]:
            ...
            raise Exception(
                "Error in expand() go back and check how images are resized"
            )
    ...
    def uniform_size(self, x):
        ...
        tup_val1 = round(((width - x.size()[2]) / 2) - 0.1)
        tup_val2 = round(((width - x.size()[2]) / 2) + 0.1)
        tup_val3 = round(((height - x.size()[1]) / 2) - 0.1)
        tup_val4 = round(((height - x.size()[1]) / 2) + 0.1)
