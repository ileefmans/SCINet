...
class FaceAlign:
    ...
    def forward(self):
        ...
        box_ims, box_list = self.annotation_extract(
            self.sample, self.height, self.width
        )
        aligned_boxes = []
        for im in box_ims:
            aligned_box = self.face_align.align(
                self.rotate(im[0], angle), gray, facebox
            )
