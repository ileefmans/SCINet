...
def get_args():
    ...
    parser.add_argument(
        ...
        help="facial landmark predictor from dlib",
    )
    parser.add_argument(
        "--detector",
        type=str,
        default="HOG",
        help='type of facial detector model used, "HOG" for HOG detector, "CNN" for CNN detector',
    )
    parser.add_argument(
        "--local",
        type=bool,
        default=False,
        help="False if running on AWS, True if running locally",
    )
    parser.add_argument(
        "--local_pickle_path",
        type=str,
        default="/Users/ianleefmans/Desktop/Insight/Project/SCINet/skinConditionDetect/pickle/simple_train_dict.pkl",
    ...
    parser.add_argument(
        ...
        type=bool,
        default=True,
        help="True: return samples condusive for geometric transform, False: return smaples condusive for deep learning",
    )
...
class RunSCINet10:
    ...
    def confidence(self, landmarks1, landmarks2, box1, box2):
        ...
        for i in range(len(landmarks1)):
            distance1 = self.euclidean_distance(
                box1[0], box1[1], landmarks1[i][0], landmarks1[i][1]
            )
            distance2 = self.euclidean_distance(
                box1[0], box1[1], landmarks2[i][0], landmarks2[i][1]
            )
            ...
            for j in range(len(boxes2)):
                ...
                if IoU > 0:
                    ...
                    confidence = self.confidence(
                        landmarks1[0], landmarks2[0], box_list1[i], box_list2[j]
                    )
            ...
            try:
                ...
                if len(matched_boxes) > 0:
                    for i in matched_boxes:
                        new_row = {
                            "metric": matched_boxes[i][0],
                            "IoU": matched_boxes[i][1],
                            "confidence": matched_boxes[i][2],
                        }
